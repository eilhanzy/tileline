...
fn bench_tuning_from_adapter_info(c: &mut Criterion) {
    let adapters = [
        (
            "Apple M3 Pro",
            make_adapter_info("Apple M3 Pro", DeviceType::IntegratedGpu),
        ),
        (
            "Apple M2",
            make_adapter_info("Apple M2", DeviceType::IntegratedGpu),
        ),
        (
            "NVIDIA RTX 4090",
            make_adapter_info("NVIDIA RTX 4090", DeviceType::DiscreteGpu),
        ),
        (
            "AMD RX 7900 XTX",
            make_adapter_info("AMD RX 7900 XTX", DeviceType::DiscreteGpu),
        ),
        (
            "Intel UHD 770",
            make_adapter_info("Intel UHD 770", DeviceType::IntegratedGpu),
        ),
    ];
...
fn bench_effective_throughput(c: &mut Criterion) {
    ...
    group.bench_function("apple_ramp_early", |b| {
        b.iter(|| apple.effective_throughput_work_units_per_present(black_box(8), black_box(5)));
    });

    // Tam throughput (yüksek frame sayısı)
    group.bench_function("apple_ramp_steady", |b| {
        b.iter(|| apple.effective_throughput_work_units_per_present(black_box(8), black_box(9999)));
    });

    // Discrete GPU — ramp yok
    group.bench_function("discrete_no_ramp", |b| {
        b.iter(|| {
            discrete.effective_throughput_work_units_per_present(black_box(8), black_box(50))
        });
    });
...
fn bench_prewarm_submits(c: &mut Criterion) {
    let profiles = [
        (
            "apple",
            GmsRuntimeTuningProfile::from_adapter_info(&make_adapter_info(
                "Apple M2",
                DeviceType::IntegratedGpu,
            )),
        ),
