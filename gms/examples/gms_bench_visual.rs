    ...
    fn parse_from_env() -> Result<Self, Box<dyn Error>> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--mode"       => { let v = next_arg(&mut args, "--mode")?;       opts.mode_override      = ModeOverride::parse(&v)?;       }
                "--vsync"      => { let v = next_arg(&mut args, "--vsync")?;      opts.vsync_override     = VsyncOverride::parse(&v)?;      }
                "--multi-gpu"  => { let v = next_arg(&mut args, "--multi-gpu")?;  opts.multi_gpu_override = MultiGpuOverride::parse(&v)?;   }
                "--duration"   => { let v = next_arg(&mut args, "--duration")?;   opts.sample_duration    = parse_secs(&v, "--duration")?;  }
                "--warmup"     => { let v = next_arg(&mut args, "--warmup")?;     opts.warmup_duration    = parse_secs(&v, "--warmup")?;    }
                "--resolution" => { let v = next_arg(&mut args, "--resolution")?; opts.resolution         = parse_res(&v)?;                 }
...
enum ModeOverride { Auto, Stable, Max }
impl ModeOverride {
    fn parse(v: &str) -> Result<Self, Box<dyn Error>> {
        Ok(match v.to_ascii_lowercase().as_str() {
            "auto" => Self::Auto, "stable" => Self::Stable, "max" | "throughput" => Self::Max,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum VsyncOverride { Auto, On, Off }
impl VsyncOverride {
    fn parse(v: &str) -> Result<Self, Box<dyn Error>> {
        Ok(match v.to_ascii_lowercase().as_str() {
            "auto" => Self::Auto, "on" | "true" | "1" => Self::On, "off" | "false" | "0" => Self::Off,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum MultiGpuOverride { Auto, On, Off }
...
fn next_arg(it: &mut impl Iterator<Item = String>, flag: &str) -> Result<String, Box<dyn Error>> {
    it.next().ok_or_else(|| Box::new(SE(format!("{flag} için değer eksik"))) as _)
}
fn parse_secs(v: &str, flag: &str) -> Result<Duration, Box<dyn Error>> {
    let s = v.parse::<f64>().map_err(|_| Box::new(SE(format!("{flag}: saniye girin"))) as Box<dyn Error>)?;
    if !s.is_finite() || s < 0.0 { return Err(Box::new(SE(format!("{flag} geçersiz")))); }
...
fn parse_res(v: &str) -> Result<PhysicalSize<u32>, Box<dyn Error>> {
    ...
    let width  = w.parse::<u32>().map_err(|_| Box::new(SE(format!("genişlik: {w}"))) as Box<dyn Error>)?;
    let height = h.parse::<u32>().map_err(|_| Box::new(SE(format!("yükseklik: {h}"))) as Box<dyn Error>)?;
    if width == 0 || height == 0 { return Err(Box::new(SE("boyutlar sıfır olamaz".into()))); }
...
impl Font {
    fn glyph(c: char) -> Glyph {
        match c {
            ...
            // Semboller
            'σ' => [0b00000, 0b01111, 0b10001, 0b10001, 0b10001, 0b10001, 0b01110],
...
impl HudRenderer {
    ...
    fn new(device: &wgpu::Device, queue: &wgpu::Queue, surface_format: TextureFormat) -> Self {
        ...
        queue.write_texture(
            ...
            &init_pixels,
            wgpu::TexelCopyBufferLayout { offset: 0, bytes_per_row: Some(HUD_PX_W * 4), rows_per_image: Some(HUD_PX_H) },
            wgpu::Extent3d { width: HUD_PX_W, height: HUD_PX_H, depth_or_array_layers: 1 },
        );
    ...
    fn render_to_pixels(&mut self, s: &HudSnapshot) {
        ...
        for b in self.pixels.iter_mut() { *b = 0; }

        let w = HUD_PX_W as usize;
        let buf = &mut self.pixels;

        // ── Arka plan: yarı şeffaf koyu panel ─────────────────────────────────
        fill_rect(buf, w, 0, 0, HUD_PX_W as usize, HUD_PX_H as usize, [10, 12, 20, 200]);
        ...
        fill_rect(buf, w, 4, chart_y - 2, HUD_PX_W as usize - 8, 1, [60, 60, 80, 200]);
...
fn create_hud_texture(device: &wgpu::Device) -> wgpu::Texture {
    device.create_texture(&wgpu::TextureDescriptor {
        ...
        size: wgpu::Extent3d { width: HUD_PX_W, height: HUD_PX_H, depth_or_array_layers: 1 },
        mip_level_count: 1,
        sample_count: 1,
...
fn draw_frame_chart(buf: &mut [u8], bw: usize, ox: usize, oy: usize, times: &[f32]) {
    ...
    for (i, &ft) in times.iter().enumerate() {
        ...
        fill_rect(buf, bw, bar_x, oy + CHART_H - h, BAR_W.saturating_sub(0), h, color);
    }

    // 60 FPS hedef çizgisi
    let target_y = oy + CHART_H - ((TARGET_MS / max_ms) * CHART_H as f32).min(CHART_H as f32) as usize;
    fill_rect(buf, bw, ox, target_y, HUD_PX_W as usize - ox * 2, 1, [200, 200, 200, 80]);
}
...
impl VisualBenchmarkApp {
    fn new(options: CliOptions) -> Self {
        Self { options, runtime: None, exit_requested: false, summary_printed: false }
    }
}
...
impl ApplicationHandler for VisualBenchmarkApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        match VisualRuntime::new(event_loop, self.options) {
            ...
            Err(e) => { eprintln!("Başlatılamadı: {e}"); event_loop.exit(); }
        }
    }

    fn window_event(&mut self, event_loop: &ActiveEventLoop, window_id: WindowId, event: WindowEvent) {
        let Some(rt) = self.runtime.as_mut() else { return; };
        if rt.renderer.window.id() != window_id { return; }

        match event {
            WindowEvent::CloseRequested => { self.exit_requested = true; event_loop.exit(); }
            ...
            WindowEvent::Resized(size) => { rt.resize(size); }
            WindowEvent::RedrawRequested => {
                if let Err(outcome) = rt.render_frame() {
                    match outcome {
                        RenderOutcome::SurfaceLost | RenderOutcome::Outdated => { rt.renderer.reconfigure(); }
                        RenderOutcome::OutOfMemory => { eprintln!("Bellek yetersiz"); event_loop.exit(); }
                        RenderOutcome::Timeout | RenderOutcome::Other => {}
                    }
                }
                if rt.use_redraw_chaining() { rt.renderer.window.request_redraw(); }
            }
            _ => {}
        }
    }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        if let Some(rt) = self.runtime.as_ref() {
            ...
            if rt.use_redraw_chaining() {
                if rt.stats.frames == 0 { rt.renderer.window.request_redraw(); }
            } else {
                rt.renderer.window.request_redraw();
            }
            if rt.should_auto_exit() { self.exit_requested = true; event_loop.exit(); }
        } else {
            event_loop.set_control_flow(ControlFlow::Wait);
        }
        if self.exit_requested { event_loop.exit(); }
    }

    fn exiting(&mut self, _: &ActiveEventLoop) {
        if self.summary_printed { return; }
...
impl VisualRuntime {
    fn new(event_loop: &ActiveEventLoop, options: CliOptions) -> Result<Self, Box<dyn Error>> {
        ...
        let pacing_mode = choose_pacing_mode(adapter_profile.as_ref(), &renderer.adapter_info, options);
        ...
        let timing_cap = renderer.runtime_tuning.benchmark_timing_capacity.max(16_384);
        let title_interval = Duration::from_millis(renderer.runtime_tuning.benchmark_title_update_interval_ms.max(100));
        let mut stats = FrameStats::new(renderer.size, timing_cap, title_interval);
        stats.last_title_update = Instant::now();
        let session_start = Instant::now();
        let sample_started_at = if options.warmup_duration.is_zero() { Some(session_start) } else { None };

        Ok(Self {
            renderer, multi_gpu, stats, adapter_profile, gms_inventory, pacing_mode,
            options, session_start, sample_started_at, sample_finished: false,
        })
    }

    fn resize(&mut self, size: PhysicalSize<u32>) {
        self.renderer.resize(size);
        if let Some(mg) = self.multi_gpu.as_mut() { mg.resize(size.width, size.height); }
    ...
    fn render_frame(&mut self) -> Result<(), RenderOutcome> {
        ...
        let secondary_wu = self.multi_gpu.as_mut().map(|mg| mg.submit_frame()).unwrap_or(0);
    ...
    fn build_snapshot(&self) -> HudSnapshot {
        ...
        let sec_wu = self.multi_gpu.as_ref().map(|mg| mg.secondary_work_units_per_present()).unwrap_or(0);

        let gpu_name = self.adapter_profile.as_ref()
    ...
    fn update_title(&mut self, instant_fps: f64, avg_fps: f64) {
        ...
        self.renderer.window.set_title(&format!(
            ...
            self.phase_label(), instant_fps, avg_fps, score
        ));
    }
    ...
    fn finish(self) -> BenchmarkSummary {
        ...
        let adapter_name = self.adapter_profile.as_ref()
            .map(|p| p.name.clone())
            .unwrap_or_else(|| self.renderer.adapter_info.name.clone());

        let score = compute_render_benchmark_score(&computed, gms_score, self.pacing_mode, &self.renderer.runtime_tuning);
        let (est_cu, cu_short, cu_disp) = self.adapter_profile.as_ref().map(|p| {
            let (sh, cnt) = p.compute_unit_summary();
            (Some(cnt), Some(sh), Some(p.compute_unit_kind.display_label()))
        ...
        BenchmarkSummary {
            ...
            compute_unit_source: self.adapter_profile.as_ref().map(|p| p.compute_unit_source.short_label()),
    ...
    fn tick_phase(&mut self, now: Instant) {
        if self.sample_finished || self.sample_started_at.is_some() { return; }
    ...
    fn tick_phase_after(&mut self, now: Instant) {
        if self.sample_finished { return; }
        if let Some(s) = self.sample_started_at {
            if now.duration_since(s) >= self.options.sample_duration { self.sample_finished = true; }
        }
    }

    /// Kısa faz etiketi (4 karakter — bitmap font dostu).
    fn phase_label(&self) -> &'static str {
        if self.sample_finished { "DONE" }
        else if self.sample_started_at.is_some() { "SMPL" }
        else { "WARM" }
    }
    ...
    fn use_redraw_chaining(&self) -> bool { matches!(self.pacing_mode, BenchmarkPacingMode::Stable) }
    fn should_auto_exit(&self) -> bool { self.sample_finished }
}
...
#[derive(Debug, Clone, Copy)]
struct ThroughputBurst { work_units_per_present: u32, offscreen_target_ring_len: usize }
...
impl Renderer {
    fn new(event_loop: &ActiveEventLoop, options: CliOptions) -> Result<Self, Box<dyn Error>> {
        ...
        let (device, queue) = pollster::block_on(adapter.request_device(
            &wgpu::DeviceDescriptor { label: Some("gms-bench-visual-dev"), ..Default::default() },
        ))?;
        ...
        let throughput_burst = select_throughput_burst(&adapter_info, &runtime_tuning, options, config.present_mode);
        // HUD renderer, yüzey formatı belirlendikten sonra oluşturulur.
        let hud = HudRenderer::new(&device, &queue, config.format);

        let mut renderer = Self {
            window, _instance: instance, surface, adapter_info, device, queue,
            config: config.clone(), size, present_mode: config.present_mode,
            clear_phase: 0.0, throughput_burst,
            throughput_targets: Vec::new(), throughput_target_cursor: 0,
            runtime_tuning, presented_frames: 0, hud,
        };
    ...
    fn resize(&mut self, size: PhysicalSize<u32>) {
        if size.width == 0 || size.height == 0 { self.size = size; self.throughput_targets.clear(); return; }
        self.size = size;
    ...
    fn render(&mut self, snapshot: HudSnapshot) -> Result<u32, RenderOutcome> {
        if self.size.width == 0 || self.size.height == 0 { return Ok(1); }

        let frame = self.surface.get_current_texture().map_err(RenderOutcome::from)?;
        ...
        let mut encoder = self.device.create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("gms-bench-visual-enc") }
        );
        ...
        self.hud.update(&self.queue, &snapshot, self.size.width, self.size.height);
    ...
    fn work_units_per_present(&self) -> u32 {
        self.throughput_burst.map(|b| b.work_units_per_present).unwrap_or(1)
    }
    fn effective_work_units_per_present(&self) -> u32 {
        self.runtime_tuning.effective_throughput_work_units_per_present(
            self.work_units_per_present(), self.presented_frames,
        )
    }
    fn current_throughput_target(&mut self) -> Option<&wgpu::Texture> {
        if self.throughput_targets.is_empty() { return None; }
        let idx = self.throughput_target_cursor % self.throughput_targets.len();
        self.throughput_target_cursor = (self.throughput_target_cursor + 1) % self.throughput_targets.len();
        self.throughput_targets.get(idx)
    }
    fn ensure_throughput_target_ring(&mut self) {
        if self.size.width == 0 || self.size.height == 0 { self.throughput_targets.clear(); return; }
        let desired = self.throughput_burst.map(|b| b.offscreen_target_ring_len.max(1)).unwrap_or(1);
        let needs = self.throughput_targets.len() != desired || self.throughput_targets.iter().any(|t| {
            let e = t.size(); e.width != self.size.width || e.height != self.size.height
        });
        if !needs { return; }
        self.throughput_targets.clear();
        for _ in 0..desired {
            self.throughput_targets.push(self.device.create_texture(&wgpu::TextureDescriptor {
                ...
                dimension: wgpu::TextureDimension::D2, format: self.config.format,
    ...
    fn prewarm_throughput_resources(&mut self) {
        if self.work_units_per_present() <= 1 || self.runtime_tuning.throughput_startup_prewarm_submits == 0 { return; }
        self.ensure_throughput_target_ring();
        if self.throughput_targets.is_empty() { return; }
        let count = self.runtime_tuning.startup_prewarm_submits_for_ring(
            self.work_units_per_present(), self.throughput_targets.len(),
        );
        for _ in 0..count {
            let Some(target) = self.current_throughput_target() else { break; };
            let tv = target.create_view(&wgpu::TextureViewDescriptor::default());
            let mut enc = self.device.create_command_encoder(
                &wgpu::CommandEncoderDescriptor { label: Some("gms-bench-visual-prewarm") }
            );
...
struct FrameTiming { display_fps: f64, avg_fps: f64, should_update_title: bool }

impl FrameStats {
    fn new(resolution: PhysicalSize<u32>, timing_capacity: usize, title_update_interval: Duration) -> Self {
        let now = Instant::now();
        Self {
            benchmark_start: now, last_frame_presented_at: None,
            last_title_update: now, title_update_interval,
            frames: 0,
            ...
            peak_fps: 0.0, resolution,
        }
    }
    fn reset_measurement(&mut self, now: Instant) {
        self.benchmark_start = now; self.last_frame_presented_at = None;
        self.last_title_update = now; self.frames = 0;
        self.frame_intervals_ms.clear(); self.render_durations_ms.clear();
        self.recent_frame_ms.clear(); self.peak_fps = 0.0;
    }
    fn record_frame(&mut self, frame_start: Instant, frame_end: Instant, work_units: u32) -> FrameTiming {
        ...
        if let Some(last) = self.last_frame_presented_at {
            ...
            if interval_ms.is_finite() && interval_ms > 0.0 {
                ...
                if self.recent_frame_ms.len() >= 120 { self.recent_frame_ms.pop_front(); }
        ...
        if should_update_title { self.last_title_update = frame_end; }
        FrameTiming { display_fps, avg_fps, should_update_title }
    }
    fn avg_fps(&self) -> f64 {
        self.frames as f64 / self.benchmark_start.elapsed().as_secs_f64().max(1e-9)
    }
    fn smoothed_fps_recent(&self, n: usize) -> Option<f64> {
        if n == 0 || self.frame_intervals_ms.is_empty() { return None; }
        ...
        if avg > 0.0 { Some(1000.0 / avg) } else { None }
    }
    fn compute_summary(self) -> ComputedFrameStats {
        ...
        ComputedFrameStats {
            total_frames: self.frames, elapsed, avg_fps, peak_fps: self.peak_fps,
            avg_frame_ms: avg_ms, p95_frame_ms: p95_ms, p99_frame_ms: p99_ms,
            low_1_percent_fps: low1pct, frame_time_stddev_ms: stddev,
            avg_work_ms: avg_work, p95_work_ms: p95_work, p99_work_ms: p99_work,
            work_time_stddev_ms: wstddev, resolution: self.resolution,
        }
    }
}
...
fn stddev(v: &[f64]) -> Option<f64> { stddev_fn(v) }
...
fn select_present_mode(modes: &[PresentMode], prefer_stable: bool, vsync: VsyncOverride) -> PresentMode {
    if matches!(vsync, VsyncOverride::On) {
        for p in [PresentMode::AutoVsync, PresentMode::Fifo, PresentMode::Mailbox] {
            if modes.contains(&p) { return p; }
        }
    }
    if matches!(vsync, VsyncOverride::Off) {
        for p in [PresentMode::AutoNoVsync, PresentMode::Immediate, PresentMode::Mailbox] {
            if modes.contains(&p) { return p; }
        }
    }
    let stable     = [PresentMode::AutoVsync, PresentMode::Fifo, PresentMode::Mailbox, PresentMode::AutoNoVsync, PresentMode::Immediate];
    let throughput = [PresentMode::AutoNoVsync, PresentMode::Immediate, PresentMode::Mailbox, PresentMode::AutoVsync, PresentMode::Fifo];
...
fn build_multi_gpu_workload_request(renderer: &Renderer, opts: CliOptions) -> MultiGpuWorkloadRequest {
    ...
    let ms = match opts.mode_override { ModeOverride::Stable => 16.67, ModeOverride::Max => 0.26, ModeOverride::Auto => if renderer.work_units_per_present() > 1 { 0.50 } else { 8.33 } };
    MultiGpuWorkloadRequest {
        ...
        bytes_per_sampled_job: 4096, bytes_per_object: 256, bytes_per_physics_job: 1024,
        bytes_per_ui_job: 512, bytes_per_post_fx_job: 1024,
        processed_texture_bytes_per_frame: px.saturating_mul(4).clamp(512*1024, 64*1024*1024),
        base_workgroup_size: 64, target_frame_budget_ms: ms,
    }
}
fn match_inventory_profile(inv: &GpuInventory, ai: &wgpu::AdapterInfo) -> Option<gms::GpuAdapterProfile> {
    inv.adapters.iter().find(|p| p.vendor_id == ai.vendor && p.device_id == ai.device && p.backend == ai.backend && p.name == ai.name)
        .or_else(|| inv.adapters.iter().find(|p| p.backend == ai.backend && p.vendor_id == ai.vendor && p.name == ai.name))
        .cloned()
}
...
struct ComputedFrameStats {
    total_frames: u64, elapsed: Duration,
    avg_fps: f64, peak_fps: f64,
    avg_frame_ms: f64, p95_frame_ms: f64, p99_frame_ms: f64,
    low_1_percent_fps: f64, frame_time_stddev_ms: f64,
    avg_work_ms: f64, p95_work_ms: f64, p99_work_ms: f64,
    work_time_stddev_ms: f64, resolution: PhysicalSize<u32>,
}

/// Benchmark sonu performans skoru ve alt faktörler.
#[derive(Debug, Clone, Copy)]
struct RenderScore {
    score: u64, tier: &'static str,
    fps_term: f64, stability_factor: f64, tail_factor: f64,
    present_stability_factor: f64, present_tail_factor: f64,
    work_stability_factor: f64, work_tail_factor: f64,
    work_stability_blend: f64, resolution_factor: f64, gms_factor: f64,
}

fn compute_render_benchmark_score(s: &ComputedFrameStats, gms_hw: u64, mode: BenchmarkPacingMode, tuning: &GmsRuntimeTuningProfile) -> RenderScore {
    let px = s.resolution.width.max(1) as f64 * s.resolution.height.max(1) as f64;
    let res_f = (px / (1280.0 * 720.0)).sqrt().clamp(0.75, 3.0);
    let avg_ms = s.avg_frame_ms.max(0.001); let p95_ms = s.p95_frame_ms.max(avg_ms); let p99_ms = s.p99_frame_ms.max(p95_ms);
    let aw = s.avg_work_ms.max(0.001); let p95w = s.p95_work_ms.max(aw); let p99w = s.p99_work_ms.max(p95w);
    let ps = (avg_ms / p95_ms).clamp(0.35, 1.0); let pt = (avg_ms / p99_ms).clamp(0.25, 1.0);
    let ws = (aw / p95w).clamp(0.35, 1.0);       let wt = (aw / p99w).clamp(0.25, 1.0);
    let blend = if matches!(mode, BenchmarkPacingMode::MaxThroughput) { tuning.throughput_work_stability_blend.clamp(0.0,1.0) } else { 0.0 };
    let stab = lerp(ps, ws, blend); let tail = lerp(pt, wt, blend);
    ...
    RenderScore { score, tier: score_tier(score), fps_term, stability_factor: stab, tail_factor: tail,
        present_stability_factor: ps, present_tail_factor: pt, work_stability_factor: ws, work_tail_factor: wt,
        work_stability_blend: blend, resolution_factor: res_f, gms_factor: gms_f }
}
fn score_tier(s: u64) -> &'static str { match s { 60_000.. => "S", 35_000.. => "A", 20_000.. => "B", 10_000.. => "C", _ => "D" } }

// ── Pacing modu ───────────────────────────────────────────────────────────────

#[derive(Debug, Clone, Copy)]
enum BenchmarkPacingMode { Stable, MaxThroughput }

fn choose_pacing_mode(profile: Option<&gms::GpuAdapterProfile>, ai: &wgpu::AdapterInfo, opts: CliOptions) -> BenchmarkPacingMode {
    match opts.mode_override { ModeOverride::Stable => return BenchmarkPacingMode::Stable, ModeOverride::Max => return BenchmarkPacingMode::MaxThroughput, ModeOverride::Auto => {} }
    if let Some(p) = profile { if matches!(p.memory_topology, gms::MemoryTopology::Unified) { return BenchmarkPacingMode::Stable; } }
    if matches!(ai.device_type, wgpu::DeviceType::IntegratedGpu) { BenchmarkPacingMode::Stable } else { BenchmarkPacingMode::MaxThroughput }
}
...
struct BenchmarkSummary {
    ...
    estimated_compute_units: Option<u32>, compute_unit_short_label: Option<&'static str>,
    compute_unit_display_label: Option<&'static str>, compute_unit_source: Option<&'static str>,
    resolution: PhysicalSize<u32>, present_mode: PresentMode,
    total_frames: u64, elapsed: Duration,
    avg_fps: f64, peak_fps: f64, avg_frame_ms: f64, p95_frame_ms: f64, p99_frame_ms: f64,
    low_1_percent_fps: f64, frame_time_stddev_ms: f64,
    avg_work_ms: f64, p95_work_ms: f64, p99_work_ms: f64, work_time_stddev_ms: f64,
    gms_hardware_score: u64, total_discovered_gpus: usize,
    render_score: u64, render_tier: &'static str, score_breakdown: RenderScore,
    work_units_per_present: u32, multi_gpu: Option<MultiGpuExecutorSummary>,
    pacing_mode: BenchmarkPacingMode, warmup_duration: Duration, sample_duration: Duration,
    mode_override: ModeOverride, vsync_override: VsyncOverride, multi_gpu_override: MultiGpuOverride,
}

fn print_summary(s: &BenchmarkSummary) {
    println!();
    println!("=== GMS Grafik Benchmark Özeti ===");
    println!("Adapter: {} | Backend: {} | Tür: {}", s.adapter_name, s.backend, s.device_type);
    if let (Some(cu), Some(sl), Some(dl), Some(src)) = (s.estimated_compute_units, s.compute_unit_short_label, s.compute_unit_display_label, s.compute_unit_source) {
        println!("Tahmini {}: {} {} (kaynak: {})", dl, cu, sl, src);
    }
    println!("Çözünürlük: {}x{} | Present: {:?} | Pacing: {:?}", s.resolution.width, s.resolution.height, s.present_mode, s.pacing_mode);
    if s.work_units_per_present > 1 { println!("Throughput burst: ×{} WU/present", s.work_units_per_present); }
    println!("Warmup: {:.2}s | Örnek: {:.2}s | Frame: {} | Süre: {:.3}s", s.warmup_duration.as_secs_f64(), s.sample_duration.as_secs_f64(), s.total_frames, s.elapsed.as_secs_f64());
    println!("FPS: ort {:.2} | tepe {:.2} | 1% alt {:.2}", s.avg_fps, s.peak_fps, s.low_1_percent_fps);
    println!("Frame-time(ms): ort {:.3} | p95 {:.3} | p99 {:.3} | σ {:.3}", s.avg_frame_ms, s.p95_frame_ms, s.p99_frame_ms, s.frame_time_stddev_ms);
    println!("Work-time(ms):  ort {:.3} | p95 {:.3} | p99 {:.3} | σ {:.3}", s.avg_work_ms, s.p95_work_ms, s.p99_work_ms, s.work_time_stddev_ms);
    println!("GMS HW skoru: {} | keşfedilen GPU: {}", s.gms_hardware_score, s.total_discovered_gpus);
    println!("Render skoru: {} [{}]", s.render_score, s.render_tier);
    println!("Faktörler => fps {:.1} | rez {:.3} | kararlılık {:.3} | kuyruk {:.3} | gms {:.3}", s.score_breakdown.fps_term, s.score_breakdown.resolution_factor, s.score_breakdown.stability_factor, s.score_breakdown.tail_factor, s.score_breakdown.gms_factor);
    if let Some(mg) = &s.multi_gpu {
        println!("Multi-GPU: {} → {} | WU/present: {} | toplam WU: {}", mg.primary_adapter_name, mg.secondary_adapter_name, mg.secondary_work_units_per_present, mg.total_secondary_work_units);
        println!("Projeksiyon: tek {:.3}ms → çok {:.3}ms | beklenen kazanç: {:.2}%", mg.estimated_single_gpu_frame_ms, mg.estimated_multi_gpu_frame_ms, mg.projected_score_gain_pct);
    }
}

// ── Hata türü ─────────────────────────────────────────────────────────────────

#[derive(Debug, Clone)] struct SE(String);
impl fmt::Display for SE { fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str(&self.0) } }
impl Error for SE {}

#[derive(Debug, Clone, Copy)]
enum RenderOutcome { Timeout, Outdated, SurfaceLost, OutOfMemory, Other }
impl From<SurfaceError> for RenderOutcome {
    fn from(e: SurfaceError) -> Self {
        match e { SurfaceError::Timeout => Self::Timeout, SurfaceError::Outdated => Self::Outdated, SurfaceError::Lost => Self::SurfaceLost, SurfaceError::OutOfMemory => Self::OutOfMemory, SurfaceError::Other => Self::Other }
    }
}
